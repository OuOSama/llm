...
def load_model_for_inference(checkpoint_path: str) -> LLM:
    ...
    config = LLMConfig(**ckpt["config"])  # rebuild config จาก dict ที่เซฟไว้ตรงๆ ไม่ต้องพิมพ์ใหม่
    model = LLM(config)
...
def generate_text(
    model: LLM, prompt: str, max_new_tokens: int = 100, temperature: float = 0.8, top_k: int = 50
) -> str:
...
if __name__ == "__main__":
    ...
    text = generate_text(model, prompt="OuOSama: Hello! Who are you?\nKING RICHARD II: I am the King, and your name is")
    print(text)